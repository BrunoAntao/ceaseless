...
export function Intersect(fig1, fig2) {
  ...
  let edges = edgify(fig1, fig2a);
  let polygons = polygonate(edges);
  let filteredPolygons = filterPolygons(polygons, fig1, fig2a, "intersect");
...
function edgify(fig1, fig2) {
  ...
  let primEdges = getEdges(fig1).concat(getEdges(fig2));
  let secEdges = [];
  //check every edge
  for (let i = 0; i < primEdges.length; i++) {
    ...
    for (let j = 0; j < primEdges.length; j++) {
      if (i != j) {
        let interPoints = findEdgeIntersection(primEdges[i], primEdges[j]);
...
function polygonate(edges) {
  ...
  let midpoints = getMidpoints(edges);
      ...
      while ((polygon.length === 0) || (!stop)) {
        ...
        for (let j = 0; j < len; j++) {
          ...
          if (!equalEdges(edges[j], edges[currentEdge])) {
            ...
            if (p) {
              let classify = classifyPoint(p, [org, dest]);
...
function filterPolygons(polygons, fig1, fig2, mode) {
  ...
  let c1, c2;
  let point;
  let bigPolygons = removeSmallPolygons(polygons, 0.0001);
